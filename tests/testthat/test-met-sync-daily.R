...
describe("daily sync archives forecasts and extends history products", {
  it("archives forecasts (incl. seasonal) and builds history_hourly/daily", {
    ...
    built <- new.env(); built$hourly <- 0L; built$daily <- 0L
    testthat::local_mocked_bindings(
      ...
      build_history_hourly = function(...) { built$hourly <- built$hourly + 1L
                                             make_obs(n = 1) },
      build_history_daily = function(...) { built$daily <- built$daily + 1L
                                            make_obs(n = 1) }
    )
    status <- met_sync_daily(site, now = now,
                             config = pipeline_config(
                               root, forecast_sources = c("openmeteo", "seasonal",
                                                          "bom_forecast")))
  ...
  it("re-fetches within the refetch window and supersedes a changed SILO value", {
    ...
    testthat::local_mocked_bindings(
      ...
      .acquire_obs = function(source, site, window, now = NULL, ...) {
        new_obs(make_obs(n = 1, variable = "temperature_2m", value = 25,
                         source = "silo", start = day))
      }
    )
    met_sync_daily(site, now = as.POSIXct("2026-02-01", tz = "UTC"),
