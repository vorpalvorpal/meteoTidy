  ...
  it("pulls history, ingests an AWS export, makes initial fits", {
    ...
    testthat::local_mocked_bindings(
      .acquire_obs = function(source, site, window, now = NULL, ...) {
        ran$history <- ran$history + 1L
        new_obs(make_obs(n = 5, source = source))
      },
      ingest_aws_export = function(...) {
        ran$ingest <- ran$ingest + 1L
        new_obs(make_obs(n = 5, source = "site_aws"))
      },
      correct_refit = function(...) {
        ran$fit <- ran$fit + 1L
        invisible()
      },
      station_coverage = function(...) {
        tibble::tibble(
    ...
    summary <- met_backfill(site,
      now = now,
      config = pipeline_config(
        root,
        obs_sources = c("silo", "openmeteo")
      ),
    ...
    testthat::local_mocked_bindings(
      ...
      station_coverage = function(...) {
        tibble::tibble(
          ...
          completeness = c(0.95, 0.0)
        )
      } # no radiation donor nearby
    )
    summary <- met_backfill(site,
      now = as.POSIXct("2026-01-01", tz = "UTC"),
