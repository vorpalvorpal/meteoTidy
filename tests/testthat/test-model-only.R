...
describe("profile_rescale is off by default", {
  it("leaves model-only winds raw with tier 'raw' unless explicitly enabled", {
    upper <- make_obs(
      n = 3, variable = "wind_speed_80m", value = c(8, 9, 10),
      source = "openmeteo"
    )
    out <- model_only_correct(upper,
      corrected_10m = c(4, 4.5, 5),
      raw_10m = c(4, 4.5, 5)
    ) # profile_rescale default off
