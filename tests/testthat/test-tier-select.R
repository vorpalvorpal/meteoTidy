  ...
  it("returns the right tier for each overlap band", {
    ...
    expect_equal(
      tier_select(make_test_site(), "openmeteo", "temperature_2m",
                  training_summary = training_summary(overlap_months = 0,
                                                      n_pairs = 0),
                  skill_verdict = pass),
      "physical")
    expect_equal(
    ...
    expect_equal(
      tier_select(make_test_site(), "openmeteo", "temperature_2m",
                  training_summary = training_summary(overlap_months = 30,
                                                      has_archive = TRUE),
                  skill_verdict = pass),
      "emos")
  })
